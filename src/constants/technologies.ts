...
const technologies: Technology[] = [
  ...
  },
  {
    ...
    certificationURL: 'https://graphacademy.neo4j.com/c/ac92de95-bc13-4e3d-9776-8723c0c585ad/'
  },
  {
    name: "Drizzle",
  ...
  {
    name: "Docker",
    path: "/images/technologies/docker.webp",
    type: "devops",
  },
];

const frontend = technologies.filter((technology) =>
  ["frontend", "languages"].includes(technology.type),
);
const backend = technologies.filter((technology) =>
  ["backend", "languages"].includes(technology.type),
);
const database = technologies.filter(
  (technology) => technology.type === "database",
);
const devops = technologies.filter(
  (technology) => technology.type === "devops",
);

export { frontend, backend, database, devops };
